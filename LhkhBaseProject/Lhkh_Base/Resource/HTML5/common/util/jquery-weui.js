...
+ function($) {
    ...
    $.modal_zdy_input = function(params, onOpen) {
        ...
        var tpl = '<div class="weui-dialog">'
            +'<input type="text" class="weui-input weui-prompt-input" id="weui-prompt-input" placeholder="'+(params.placeholder || '')+'" value="' + (params.input || '') + '" style="height:48px;margin:25px;font-size:18px;" />'
            +'<div class="weui-dialog__ft">' + buttonsHtml + '</div>' +
            '</div>';
    ...
    $.alert = function(text, title,buttonOK, onOK) {
        var config;
    ...
    $.prompt_zdy = function(text, title, onOK, onCancel, input) {
        ...
        var modal = $.modal_zdy({
            text: '<p class="weui-prompt-text text_left font_weight" style="padding-left:25px;color:#000">'+(config.user_name || '')+'</p>'
                ...
                +'<input type="'+(config.inputType||'text')+'" class="weui-input weui-prompt-input" id="weui-prompt-input" placeholder="'+(config.placeholder || '')+'" value="' + (config.input || '') + '" />'
                +( config.warn_msg ? '<p class="text_left" style="padding:5px 0 0 28px;text-align: left;">'+config.warn_msg+'</p>' : ''),//<i class="weui-icon-warn" style="color:#000;font-size:0.48rem;margin-bottom:5px;"></i>
    ...
    var Picker = function (params) {
        ...
        p.columnHTML = function (col, onlyItems) {
            ...
            if (col.divider) {
                ...
                var lend = p.params.cols.length;
                columnHTML += '<div style="width:'+100/lend+'%;" class="picker-items-col ' + (col.textAlign ? 'picker-items-col-' + col.textAlign : '') + ' ' + (col.cssClass || '') + '"><div class="picker-items-col-wrapper">' + columnItemsHTML + '</div></div>';
            }
        ...
        p.updateContent = function(){
            p.layout();
            p.container = $($.updatePicker(p.pickerHTML));
...
+function ($) {
    ...
    var Calendar = function (params) {
        ...
        function closeOnHTMLClick(e) {
            if (inPopover()) return;
            if (p.input && p.input.length > 0) {
                var flag = (!e.target.closest(p.params.showDiv)||(e.target.closest(p.params.showDiv)&&e.target.closest(p.params.showDiv).length==0));
                if (e.target !== p.input[0]&& flag && $(e.target).parents('.weui-picker-modal').length === 0) p.close();
            }
            else {
                if ($(e.target).parents('.weui-picker-modal').length === 0) p.close();
            }
