...
#import "UILabel+Lhkh.h"
#import <objc/runtime.h>
#import <CoreText/CoreText.h>
#import <Foundation/Foundation.h>

@implementation LhkhAttributeModel

@end

@implementation UILabel (Lhkh)

#pragma mark - ChangeLineSpaceAndWordSpace
...
#pragma mark - LhkhAttributeTextTapAction
...
- (id<LhkhAttributeTextTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<LhkhAttributeTextTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - mainFunction
- (void)lhkh_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self lhkh_getRangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self lhkh_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(lhkh_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate lhkh_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self lhkh_saveEffectDicWithRange:range];
            
            [self lhkh_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self lhkh_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)lhkh_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self lhkh_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self lhkh_getLineBounds:line point:linePoint];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                LhkhAttributeModel *model = self.attributeStrings[j];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(lhkh_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)lhkh_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)lhkh_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)lhkh_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)lhkh_saveEffectDicWithRange:(NSRange)range
{
...
- (void)lhkh_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf lhkh_getStringWithRange:range]];
            
            LhkhAttributeModel *model = [LhkhAttributeModel new];
...
- (NSString *)lhkh_getStringWithRange:(NSRange)range
{
